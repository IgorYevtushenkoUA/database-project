...
(
    ...
    student_code  integer     not null
        constraint bigunets_mark_pk_2
            primary key
        constraint fk_student
            references student
            on update cascade on delete cascade,
    vidomist_no   integer     not null
        constraint fk_vidomist
            references vidomist
            on update cascade on delete cascade,
    ...
    constraint bigunets_mark_pk
        unique (bigunets_no, vidomist_no)
);
...
alter table bigunets
    owner to postgres;


alter table bigunets_mark
    owner to postgres;
