...
public interface StudentRepository extends JpaRepository<StudentEntity, Integer>, StudentRepositoryCustom {
    ...
    @Query(value = "select s.studentCode, s.studentRecordBook, s.studentSurname, s.studentName, s.studentPatronymic, gr.eduYear as eduYear, gr.course as course, gr.trim as semestr, avg(vm.completeMark) as completeMark " +
            "from GroupEntity gr " +
            "inner join VidmomistEntity v on gr.groupCode = v.group.groupCode " +
            ...
            "where exists ( " +
            "   select s1.studentCode, s1.studentSurname, s1.studentName, s1.studentPatronymic, gr1.eduYear, gr1.course, gr1.trim  " +
            "   from GroupEntity gr1 " +
            "   inner join VidmomistEntity v1 on gr1.groupCode=v1.group.groupCode " +
            "   inner join VidomistMarkEntity vm1 on vm1.vidomistMarkId.vidomistNo = v1.vidomistNo " +
            "   inner join StudentEntity s1 on s1.studentCode = vm1.vidomistMarkId.studentCode " +
            "   where s.studentCode =s1.studentCode " +
            "   group by s1.studentCode, s1.studentSurname, s1.studentName, s1.studentPatronymic, gr1.eduYear, gr1.course, gr1.trim  " +
            "   having gr.course = max(gr1.course) " +
            "       and gr.trim = max(gr1.trim) " +
            ")" +
            "group by  s.studentCode, s.studentRecordBook, s.studentSurname, s.studentName, s.studentPatronymic ,gr.eduYear , gr.course, gr.trim  ", nativeQuery = true)
