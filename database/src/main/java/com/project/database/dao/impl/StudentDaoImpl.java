...
public class StudentDaoImpl implements StudentDao {
    ...
    public List<Object> findAllWhoHasRetakeSubjectTrimEduYear(String subjectName,
        ...
        try {
            String sql = "select s.student_surname, s.student_name, s.student_patronymic,s2.subject_name, bm.complete_mark\n" +
                    "from ((student s inner join vidomist_mark vm on s.student_code = vm.student_code)\n" +
                    "    inner join bigunets_mark bm on vm.student_code = bm.student_code)\n" +
                    "         inner join vidomist v on vm.vidomist_no = v.vidomist_no\n" +
                    "         inner join \"group\" g on g.group_code = v.group_code\n" +
                    "         inner join subject s2 on s2.subject_no = g.subject_no\n" +
                    "where s2.subject_name " + subjectName + trim + eduYear +
                    "order by " + sortType + sortGrow + " limit ? offset ?";
            System.out.println(sql);
            preparedStatement = connection.prepareStatement(sql);
            preparedStatement.setInt(index++, numberPerPage);
            preparedStatement.setInt(index++, (page - 1) * numberPerPage);
    ...
    public List<Object> findAllRetakenSubjectForStudentTrimEduYear(int studentId,
        ...
        try {
            String sql = "select s.student_surname, s.student_name, s.student_patronymic,s2.subject_name, bm.complete_mark\n" +
                    ...
                    "         inner join \"group\" g on g.group_code = v.group_code\n" +
                    "         inner join subject s2 on s2.subject_no = g.subject_no\n" +
                    "where s.student_code =  " + studentId + trim + eduYear +
                    "order by " + sortType + sortGrow + " limit ? offset ?";
            System.out.println(sql);
            preparedStatement = connection.prepareStatement(sql);
            preparedStatement.setInt(index++, numberPerPage);
    ...
    TreeMap<String, String> setParamsDebtor(String eduYear, String subjectName, String groupName, String tutorName, String trim, String course, String sortType, String sortGrow) {
        TreeMap<String, String> map = new TreeMap<>();
        eduYear = eduYear == null ? " in (select distinct(gg.edu_year) from \"group\" gg) " : " = " + eduYear + " ";
        subjectName = subjectName == null ? " " : " and subject_no in (select subject_no from subject where subject_no = " + subjectName + " ) ";
    ...
    public List<Student> findAllDebtorsByYearSubjectGroupTeacherTrimCourse(
        ...
        List<Student> students = new ArrayList<>();
        index = 1;
        TreeMap<String, String> params = setParamsDebtor(eduYear, subjectName, groupName, tutorName, trim, course, sortType, sortGrow);
        try {
            String sql = "select *\n" +
                    "from student s\n" +
                    "where exists(\n" +
                    ...
                    "                  select vidomist_no\n" +
                    "                  from vidomist\n" +
                    "                           inner join \"group\" using (group_code)\n" +
                    "                  where edu_year " + params.get("eduYear") +
                    params.get("subjectName") +
                    params.get("groupName") +
                    params.get("tutorName") +
                    params.get("trim") +
                    params.get("course") +
                    "\n" +
                    "                   " +
